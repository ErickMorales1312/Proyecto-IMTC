...
#define Flame1Pin 33     // Sensor de flama (DO) 

#define Flame2Pin 27     // Sensor de flama (DO) 

#define Flame3Pin 21     // Sensor de flama (DO) 

#define Flame4Pin 19     // Sensor de flama (DO) 

#define AlarmPin 18     // Buzzer 

#define LEDPin 14     // LED indicador 
...
const float TEMP_UMBRAL = 50.0; // Temperatura mínima para considerar fuego 
...
int tiempoAlertaSimulada = 25000; 
...
static char telemetry_topic[128] = "devices/DetectionModule/messages/events/"; 
...
static void generateTelemetryPayload(String message) 
...
  telemetry_payload = message; 

} 

 

static void sendTelemetry(String message) 
...
    int flame1 = digitalRead(Flame1Pin); 

    int flame2 = digitalRead(Flame2Pin); 

    int flame3 = digitalRead(Flame3Pin); 

    int flame4 = digitalRead(Flame4Pin); 
...
    if (flame1 == HIGH || flame2 == HIGH || flame3 == HIGH || flame4 == HIGH) 
...
    if (fuegoDetectado && temp > TEMP_UMBRAL) 
...
  Serial.println("Temperatura alta y flama presente"); 
...
  Serial.println("Enviando coordenadas: " + String(latitud) +  ", " + String(longitud)); 

  Serial.println("Con calidad: Satelites = " + String(satelites) + ", HDOP = " + String(hdop)); 

   

  int latEntero = (int)latitud; 

  int latDecimal = abs((int)((latitud - latEntero) * 10000000)); 

  int lonEntero = (int)longitud; 

  int lonDecimal = abs((int)((longitud - lonEntero) * 10000000)); 

 

  String resultado = String(latEntero) + "," + String(latDecimal) + "," + String(lonEntero) + "," + String(lonDecimal); 

 

  sendTelemetry(resultado); 
